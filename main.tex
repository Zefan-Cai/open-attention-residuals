...
MUDDFormer~\citep{wang2024muddformer} confirms this intuition empirically: ablating the value stream's dense connections causes 0.28 PPL degradation (the largest among all streams), while Q and K contribute only 0.12--0.13 each. The value stream benefits most from cross-layer information access.

We propose \textbf{per-stream delta routing}, which gives the value projection its own independent depth-routing query:
\begin{align}
...
\paragraph{Intuition.} Consider a deep layer that needs to: (1)~match token positions using structural/syntactic information from middle layers (Q/K routing), and (2)~retrieve semantic content from early layers where token meanings are less diluted (V routing). With a single shared routing, these competing preferences must compromise. Per-stream routing resolves this conflict.
...
At 100M scale, Delta-V AttnRes achieves the lowest WikiText-2 perplexity (71.11), reducing PPL by 8.5\% relative to the baseline and 1.6\% relative to standard Delta AttnRes. The improvement from Delta to Delta-V is consistent: per-stream routing allows the value stream to retrieve content from different depth layers than the Q/K positional matching, resolving a representational conflict inherent in shared routing. The additional parameter cost is negligible (12.3K, or 0.011\% of model size).
...
\begin{enumerate}
    ...
    \item \textbf{Per-stream routing unlocks further gains}: Decoupling the value stream's depth routing from Q/K allows content retrieval and positional matching to specialize independently, adding only 0.011\% parameters.
